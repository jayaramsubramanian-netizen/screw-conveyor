"""
KWS Manufacturing Screw Conveyor Calculation Engine (SI units).
"""
import math
...
def calculate(inp: ConveyorInput) -> CalculationResult:
    ...
    if inp.angle_deg > 20:
        warnings.append(f"Angle {inp.angle_deg}° > 20° — KWS capacity factor < 0.75.")
    ...
    P_empty    = power_empty_friction(D, inp.length_m, inp.speed_rpm, inp.hangers_count, Ce=0.055)
    P_material = (Q_design * inp.length_m * lam * Ks) / 367.0
    P_friction_total = (P_empty + P_material) * KWS_Fd * KWS_Ff / 2.0
    P_incline  = power_incline(Q_design, inp.length_m, inp.angle_deg)
    P_shaft    = P_friction_total + P_incline
    P_installed = P_shaft / MOTOR_EFFICIENCY
    motor_kW   = select_motor(P_installed, MOTOR_SIZES_KW)

    if inp.speed_rpm > 190:
        warnings.append("KWS recommends ≤190 RPM for bulk material handling.")
    ...
    if L10h < 20000:
        warnings.append(f"Bearing L10 {L10h:.0f} h < 20,000 h recommended.")
    ...
    return CalculationResult(
        standard="KWS", material=inp.material, inputs=inp,
