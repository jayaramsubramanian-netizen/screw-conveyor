...
import type { EngineInput, EngineResult, AxialProfileResult } from '../types/api'

// ── Default inputs (mirrors JS DEF object exactly) ────────────────
export const DEFAULT_INPUT: EngineInput = {
  type:           'screw',
...
interface CalcStore {
  inp:         EngineInput
  setInp:      (patch: Partial<EngineInput>) => void
  applyDesign: (patch: Partial<EngineInput>) => void
}
...
export function useCalculate() {
  ...
  return useQuery<EngineResult, Error>({
...
export function useAxialProfile(segments = 60) {
  const inp = useCalcStore((s) => s.inp)
  return useQuery<AxialProfileResult, Error>({
...
export function useMaterials(params?: Parameters<typeof api.getMaterials>[0]) {
  return useQuery({
...
/** Bearings */
export function useBearings(params?: Parameters<typeof api.getBearings>[0]) {
  return useQuery({
...
/** Gearboxes */
export function useGearboxes(params?: Parameters<typeof api.getGearboxes>[0]) {
  return useQuery({
...
export function useFamily(payload: Parameters<typeof api.getFamily>[0]) {
  return useQuery({
