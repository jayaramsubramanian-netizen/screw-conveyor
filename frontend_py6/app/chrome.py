...
class TopNav(QFrame):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # Tab pills — populated per-module by set_tabs(). Previously this
        # loop ran over CALC_TABS at construction, hardwiring the conveyor
        # calculator's seven tabs into the chrome for every module.
    ...
    def set_tabs(self, tabs, active: str | None = None) -> None:
        """
        Replace the tab strip with the active module's tabs.

        `tabs` is a sequence of modules.base.TabSpec. An empty sequence
        clears the strip entirely, which is what a module with no tabs
        (Mixer, the stubs) wants — previously they inherited the conveyor's
        Results / Optimizer / Checks / … pills, which did nothing when
        clicked because the tab handler was guarded on page == "calc".
        """
        ...
        for spec in tabs:
            btn = NavTabButton(spec.label)
            btn.clicked.connect(
                lambda checked, tid=spec.tab_id: self._select_tab(tid)
            )
            self._tab_row.addWidget(btn)
            self.tab_buttons[spec.tab_id] = btn
            self._tab_labels[spec.tab_id] = spec.label

        target = active if active in self.tab_buttons else (
            next(iter(self.tab_buttons), None)
        )
        if target:
            self.tab_buttons[target].setChecked(True)

    def clear_kpis(self) -> None:
        """Blank the chips — called on every module change so one module's
        numbers never linger under another module's name."""
        for chip in self.kpi_chips.values():
            chip.set_value("—", TEXT3)

    def update_fail_badge(self, n_fail: int) -> None:
        btn = self.tab_buttons.get("checks")
        if btn is None:
            return          # active module has no Checks tab
        base = self._tab_labels.get("checks", "Checks")
        btn.setText(f"{base}  ·{n_fail}" if n_fail > 0 else base)
        btn._apply_style()
